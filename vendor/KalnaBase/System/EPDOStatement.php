...
class EPDOStatement extends PDOStatement {
    ...
    public function interpolateQuery($inputParams = null) {
        ...
        $countNumKeys = substr_count($testQuery, '?');
        $countNumParams = 0;
        $countStrKeys = substr_count($testQuery, ':');
        $countStrParams = 0;
        ...
        if ($this->boundParams) {
            ...
            foreach ($this->boundParams as $key => $array) {
                ...
                if (preg_match($testParam, $testQuery)) {
                    $testQuery = preg_replace($testParam, $replValue, $testQuery, 1);
                    $this->usedParams[] = [$key => $value];
                } else {
                    $this->unusedParams[] = [$key => $value];
//                    var_dump([__LINE__ . __METHOD__=>[$key=>$value]]);
                }
            }
        }

        $this->numberOfKeys = $countNumKeys + $countStrKeys;
        $this->numberOfParams = $countNumParams + $countStrParams;
